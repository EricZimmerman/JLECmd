...
namespace JLECmd
{
    internal class Program
    {
        ...
        private static List<AutoJumpListFile> _processedFiles;
...
//                    "When true, Extra blocks information will NOT be displayed. Default is false.").SetDefault(false);
            ...
            var footer = @"Examples: JLECmd.exe -f ""C:\Temp\foobar.lnk""" + "\r\n\t " +
                         @" JLECmd.exe -f ""C:\Temp\somelink.lnk"" --json ""D:\jsonOutput"" --jsonpretty" + "\r\n\t " +
                         @" JLECmd.exe -d ""C:\Temp"" --csv ""c:\temp\Lnk_out.tsv"" --html c:\temp --xml c:\temp\xml -q" + "\r\n\t " +
                         @" JLECmd.exe -f ""C:\Temp\some other link.lnk"" --nid --neb " + "\r\n\t " +
            ...
            _processedFiles = new List<AutoJumpListFile>();
            ...
            {
                AutoJumpListFile adjl = null;

                try
                {
                    adjl = ProcessFile(_fluentCommandLineParser.Object.File);
                    if (adjl != null)
                    {
                        _processedFiles.Add(adjl);
                    }
                }
            ...
            else
            {
                ...
                try
                {
                    var mask = "*.automaticDestinations-ms";
...
//                    }
        ...
        private static CsvOut GetCsvFormat(LnkFile lnk)
        {
            var csOut = new CsvOut
            {
                ...
                TargetCreated = lnk.Header.TargetCreationDate.Year == 1601 ? string.Empty : lnk.Header.TargetCreationDate.ToString(),
                TargetModified = lnk.Header.TargetModificationDate.Year == 1601 ? string.Empty : lnk.Header.TargetModificationDate.ToString(),
                TargetAccessed = lnk.Header.TargetLastAccessedDate.Year == 1601 ? string.Empty : lnk.Header.TargetLastAccessedDate.ToString(),
        ...
        private static AutoJumpListFile ProcessFile(string jlFile)
        {
            ...
            try
            {
                ...
                _logger.Info($"  Actual DestList entries {autoDest.DestList.Count.ToString("N0")}");
                ...
                foreach (var autoDestList in autoDest.DestList)
                {
